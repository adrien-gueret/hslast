(function(window) {
    ...
    function getHearthstoneCardsByIdsFromLocalStorage(ids, locale) {
        try {
            return JSON.parse(localStorage.getItem('hsCardsByIds.' + JSON.stringify(ids) + '.' + locale));
        } catch (e) {
            return null;
        };
    }
    ...
    function saveHearthstoneCardsByIdsInLocalStorage(cards, ids, locale) {
        localStorage.setItem('hsCardsByIds.' + JSON.stringify(ids) + '.' + locale, JSON.stringify(cards));
    }
    ...
    Client.prototype.executeGetCardsRequest = function (queryParams) {
        ...
        url += Object.keys(queryParams).map(function(paramName) {
        ...
        return that.checkAccessTokenExpiration().then(function() {
            return that.catchRequestError(fetch(url, requestOptions)).then(function(cardData) {
    ...
    Client.prototype.fetchToken = function() {
        ...
        var oauthHeaders = new Headers({
            Authorization: 'Basic M2IyZjhjN2JmOTBkNDU5Nzk2N2RmNmU4MGRkY2ZmZGQ6blJJTWJyQ0t1Q2IyQXVTV282b2pxeTdISHlvQ1kzV1Y=',
        });
